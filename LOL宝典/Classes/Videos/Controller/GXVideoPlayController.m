...
@interface GXVideoPlayController ()<WMPlayerDelegate, GXVideoInfoViewDelegate>
{
    WMPlayer  *wmPlayer;
    CGRect     playerFrame;
}
...
@implementation GXVideoPlayController
...
- (void)viewDidLoad
{
    ...
    self.videoInfo.url = self.URLString;
    playerFrame = CGRectMake(0, 0, kScreenWidth, (kScreenWidth)*(0.65));
    ...
    if (dictArr && [dictArr isKindOfClass:[NSArray class]]) {
        ...
        [downLoadVideoArr enumerateObjectsUsingBlock:^(GXVideoInfo *videoInfo, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([videoInfo.url isEqualToString:self.videoInfo.url]) {
                self.videoInfo.hasCaching = videoInfo.hasCaching;
            }
        }];
    }
    GXVideoInfoView *downloadView = [[GXVideoInfoView alloc] init];
    downloadView.frame = CGRectMake(0, (kScreenWidth)*(0.65), kScreenWidth, 60);
